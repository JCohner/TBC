...
void CANIntHandler(void)
{
    ...
    {
        // Getting to this point means that the TX interrupt occurred on message object 3
...
int main(void)
{
    ...
    for(;;)
    {
        ...
        if(g_bRXFlag)
        {
            ...
            {
                //Maybe add a light turn on when this happens
            }
            ...
            else if(pui8Msg1Data[3] == 0xaa)
            {
                ...
                int tempPos = qeiPosition;

                PWMPulseWidthSet(PWM0_BASE, PWM_OUT_7, PERIOD/2);

                while(200 != stepCount){;}
                /*
                totalPosition = 0; // TODO: This is only for testing!!! REMOVE LATER

                while(totalPosition < 300)
                {
                    qeiPosition = QEIPositionGet(QEI0_BASE);
                    stepDelta = abs(tempPos - qeiPosition);

                    totalPosition += stepDelta;
